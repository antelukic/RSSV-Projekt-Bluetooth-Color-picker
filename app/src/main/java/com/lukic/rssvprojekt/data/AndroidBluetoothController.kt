...
internal class AndroidBluetoothController(
    ...
    override fun startDiscovery() {
        if (!hasPermission(Manifest.permission.BLUETOOTH_SCAN)) {
            _errors.tryEmit("Doesn't have permission")
            return
        }
                ...
                currentClientSocket?.let {
                    ...
                    emitAll(
                        service
                            ...
                            .map(ConnectionResult::TransferSucceeded)
                    )
                }
            }
        }.onCompletion {
            ...
            currentClientSocket?.let { socket ->
                try {
                    ...
                    BluetoothDataTransferService(socket).also {
                        ...
                        emitAll(
                            it.listenForIncomingMessages().map(ConnectionResult::TransferSucceeded)
                        )
                    }
                } catch (e: IOException) {
                    e.printStackTrace()
    ...
    override suspend fun trySendMessage(message: String): String? {
        if (hasPermission(Manifest.permission.BLUETOOTH_CONNECT).not()) return null
